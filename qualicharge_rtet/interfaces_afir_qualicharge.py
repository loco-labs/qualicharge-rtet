...
def creation_pandas_stations(
    ...
    if only_mandatory:
        return stations[
            ["p_cum", "p_max", ID_STATION_ITINERANCE, GEOM, NODE_ID, NATURE]
        ]
    return stations


def export_stations_parcs(
    graph: gnx.GeoGraph, only_mandatory: bool = False
) -> pd.DataFrame:
...
def filter_stations_parcs(
    rtet_edges: gpd.GeoDataFrame,
    rtet_nodes: gpd.GeoDataFrame,
